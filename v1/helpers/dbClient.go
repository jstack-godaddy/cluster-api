...
func NewDBClient() (cdb *ClusterDB, err error) {
...
func (db ClusterDB) GetProjectsByTeam(team string) (projects []Project, err error) {
	...
	team_query := `SELECT team_id FROM teams WHERE team_snow=?`
	var team_id int
	row := db.QueryRowx(team_query, team)
	_ = row.Scan(&team_id)

	projects_query := `SELECT * FROM projects WHERE team_id=?`
	db.Select(&projects, projects_query, team_id)
